...
## Один приказ в слоте. Цель фиксируется КЛЕТКОЙ на момент планирования (no-retarget).
...
var relative: bool = false               # true → эффект целится в (текущая_клетка + offset),
                                         # т.е. нон-таргет: направление/дистанция фиксированы, а точка
                                         # считается от ТЕКУЩЕЙ позиции (устойчиво к отбросу/сдвигу)
