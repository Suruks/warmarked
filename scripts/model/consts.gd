...
# --- Новые умения ---
# Держись подальше (Охотник): СТОЙКА на 1 раунд (как Засада/Рефлексы). Пока взведена, враг,
# ВОШЕДШИЙ в соседнюю клетку (Chebyshev 1) с этим Охотником, получает урон и отбрасывается на 1
# клетку от него. Стойка держит весь свой раунд и может сработать не раз; сбрасывается на след. раунде.
const STAY_AWAY_MANA := 2
const STAY_AWAY_DMG := 4
