...
# Область раскладки боя/меню. Ширина/высота — из Layout (пересчитаны под реальный экран в
# _recompute_layout), центрирована по горизонтали. Все элементы боя/меню — её дети: позиции
...
# Пересчитывает раскладку (Layout.recompute) и переприменяет её ко всем элементам под
# ФАКТИЧЕСКИЙ размер вьюпорта — в его собственных координатах верстают все Layout.*-размеры
# и весь остальной UI. Висит на Viewport.size_changed (а не только Window.size_changed):
# смена content_scale_aspect в _adapt_stretch_for_orientation тоже меняет размер вьюпорта и
# должна попасть сюда, даже если реальное окно ОС при этом не резайзилось.
func _recompute_layout() -> void:
	if _content == null:
		return   # раскладка ещё не построена (первый вызов — из _build_layout)
