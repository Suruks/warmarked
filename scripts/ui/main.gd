...
# ВРЕМЕННЫЙ диагностический оверлей: сырые числа растяжения прямо на экране (на телефоне
# консоль браузера не открыть). Удалить вместе с _update_debug_overlay после починки бага.
...
# ВРЕМЕННО: см. объявление _debug_lbl выше.
func _update_debug_overlay() -> void:
	if _debug_lbl == null:
		return
	var aspect_names := {0: "IGNORE", 1: "KEEP", 2: "KEEP_WIDTH", 3: "KEEP_HEIGHT", 4: "EXPAND"}
	var aspect: int = get_window().content_scale_aspect
	var content_sz: String = str(_content.size) if _content != null else "-"
	_debug_lbl.text = "win_get_size=%s\nvisible_rect=%s\nscreen_size(win)=%s\naspect=%s scale_mode=%s\ncell=%.1f SCREEN_W=%.0f SCREEN_H=%.0f\n_content.size=%s" % [
		DisplayServer.window_get_size(),
		get_viewport().get_visible_rect().size,
		DisplayServer.screen_get_size(),
		aspect_names.get(aspect, str(aspect)), get_window().content_scale_mode,
		Layout.cell_size, Layout.SCREEN_W, Layout.SCREEN_H,
		content_sz,
	]


func _build_layout() -> void:
	# ВРЕМЕННЫЙ диагностический оверлей — на root напрямую (не в _content), верхний левый угол,
	# поверх всего (z_index), чтобы был виден независимо от того, что сломано в остальной раскладке.
	var dbg_bg := Panel.new()
	var dbg_sb := StyleBoxFlat.new()
	dbg_sb.bg_color = Color(0, 0, 0, 0.8)
	dbg_bg.add_theme_stylebox_override("panel", dbg_sb)
	dbg_bg.position = Vector2(2, 2)
	dbg_bg.size = Vector2(400, 150)
	dbg_bg.z_index = 100
	add_child(dbg_bg)
	_debug_lbl = Label.new()
	_debug_lbl.add_theme_font_size_override("font_size", 13)
	_debug_lbl.add_theme_color_override("font_color", Color.YELLOW)
	_debug_lbl.autowrap_mode = TextServer.AUTOWRAP_WORD
	_debug_lbl.position = Vector2(6, 4)
	_debug_lbl.size = Vector2(390, 142)
	_debug_lbl.z_index = 101
