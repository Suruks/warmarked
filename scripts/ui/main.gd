...
# локальный режим (только hotseat двух людей: бой с ИИ — серверный, см. _start_ai_match)
...
var _just_set_record := false            # победа только что подняла личный рекорд (для экрана победы)

# онлайн-режим (в т.ч. бой против ИИ — он тоже идёт на сервере)
var online := false
var _ai_match := false       # текущий серверный матч — против бота, а не против живого соперника
var my_index := -1
var _auto := false
var _auto_ai := false        # автопрогон: бой против ИИ вместо очереди PvP (см. _ready)
...
var _current_login := ""
# Зачем переподключаемся: и «Онлайн», и «Бой» с ИИ требуют живого сокета, поэтому после разрыва
# сначала тихо входим по токену, а затем доводим до конца именно то действие, которое просил
# игрок (см. _on_auth_ok) — иначе клик по «Бой» уносил бы в очередь PvP.
...
func _ready() -> void:
	...
	_build_layout()
	_connect_net()
	if args.has("autoclient-ai") or args.has("--autoclient-ai"):
		# Автопрогон боя с ИИ: тот же путь, что и у живого игрока (сервер ведёт матч), но
		# приказы — пустые, поэтому бот доводит матч до победы за несколько раундов.
		_auto = true
		_auto_ai = true
		_begin_auth(_arg_value(args, ["autoclient-ai", "--autoclient-ai"], "127.0.0.1"))
	elif args.has("autoclient") or args.has("--autoclient"):
		_auto = true
...
func _on_options_pressed() -> void:
	...
	dlg.add_child(box)
	# Настройки уезжают на сервер один раз, при закрытии окна: слайдер за одно перетаскивание
	# даёт десятки value_changed, и слать пакет на каждый было бы расточительством на ровном месте.
	dlg.close_requested.connect(_close_options.bind(dlg))
	dlg.confirmed.connect(_close_options.bind(dlg))
	add_child(dlg)
	dlg.popup_centered()


func _volume_row() -> VBoxContainer:
	var col := VBoxContainer.new()
	col.add_theme_constant_override("separation", 4)
	var lbl := Label.new()
	lbl.text = "Громкость: %d%%" % roundi(Settings.volume * 100.0)
	col.add_child(lbl)
	...
	# Громкость применяется сразу: игрок слышит, что выбирает, а не после следующего боя.
	slider.value_changed.connect(func(v: float):
		Settings.set_volume(v)
		lbl.text = "Громкость: %d%%" % roundi(v * 100.0))
	# ...и по концу перетаскивания — образец звука: иначе на тихом ходу раунда ползунок
	# двигают вслепую, не понимая, что значит выбранный уровень.
...
func _close_options(dlg: AcceptDialog) -> void:
	dlg.queue_free()
	Net.save_settings()
...
func _status(text: String, show_cancel: bool = false) -> void:
	...
	if show_cancel:
		...
		btn.pressed.connect(_exit_match_to_menu)
		box.add_child(btn)
	_set_panel(box)
...
func _show_menu() -> void:
	...
	b_ai.pressed.connect(_show_difficulty)
	box.add_child(b_ai)
	...
	b_online.pressed.connect(func():
		...
		if _authed_connection:
			...
			# разрыв связи со старта — тихо переподключаемся по токену и встаём в очередь
			_pending_intent = INTENT_ONLINE
			_begin_auth(DEFAULT_HOST))
...
# Экран «Игра против ИИ» — на весь экран (как «Коллекция», а не диалогом поверх меню):
# лидерборд рекордов + выбор уровня сложности. Максимум слайдера растёт блоками по
# Difficulty.TIER — победа на старшем открытом уровне открывает следующий блок
# (см. _on_local_resolution_done/_on_ai_win), вплоть до MAX_LEVEL.
func _show_difficulty() -> void:
	_menu_art.visible = false
	_background.visible = true   # фон под окном сложности
	for c in panel_host.get_children():
		c.queue_free()
	var dp := DifficultyPanel.new()
	dp.setup(_current_login)
	dp.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	dp.offset_left = Layout.BOARD_X
	dp.offset_right = -Layout.BOARD_X
	dp.offset_top = Layout.SCORE_TOP_Y
	dp.offset_bottom = -Layout.PANEL_BOTTOM_MARGIN
	add_child(dp)
	dp.fight.connect(func():
		dp.queue_free()
		_start_ai_match())
	dp.closed.connect(func():
		dp.queue_free()
		_show_menu())


# ============================================================ бой против ИИ (на сервере)

# Бой с ИИ — серверный матч (как PvP), а не локальный: клиент только просит начать и дальше
# планирует своего игрока. Приказы бота, резолвинг и исход — на сервере, поэтому нарисовать
# себе победу (а с ней рекорд в лидерборде) клиент не может.
func _start_ai_match() -> void:
	_menu_art.visible = false
	if not Net.start_ai_match(Difficulty.level):
		# Сокет умер фоном — боя не будет, сервер единственный, кто его ведёт. Тихо
		# переподключаемся по токену и начинаем бой уже после входа (см. _pending_intent).
		_pending_intent = INTENT_AI
		_begin_auth(DEFAULT_HOST)
		return
	online = true
	_ai_match = true
	_status("Подготовка боя с ИИ…", true)


func _on_ai_matched(a_first_on_odd: bool, loadout_a: Array, loadout_b: Array, map_index: int,
		level: int, mod_seed: int) -> void:
	# Боя мы уже не ждём (нажата «Отмена» — сокет при этом жив, см. _exit_match_to_menu), а пакет
	# сервера был в пути. Матч на сервере закрыт нашим же req_leave_match — открывать его у себя
	# нельзя: приказы уходили бы в никуда, и экран завис бы на «Ход ИИ…».
	if not _ai_match:
		return
	my_index = 0   # против ИИ человек всегда за A
	online = true
	state = MatchState.new()
	state.setup(loadout_a, loadout_b, map_index)
	# Копия матча обязана совпасть с серверной до последнего HP: уровень и жребий модификаторов
	# берём из пакета сервера, а не из своих Difficulty.level/randi() — иначе бот у нас окажется
	# не тем, что резолвит сервер, и лок-степ разойдётся.
	Difficulty.apply(state, Consts.Player.B, level, mod_seed)
...
func _on_ai_match_denied(unlocked: int) -> void:
	# Сервер не дал играть этот уровень (зеркало прогресса разошлось с БД — например, играли
	# с другого устройства). Net уже поправил Difficulty.unlocked по авторитету сервера.
	_ai_match = false
	online = false
...
# когда доиграет анимация. Клиент здесь ничего не решает: он только показывает.
func _on_ai_progress(new_record: bool, new_unlock: bool) -> void:
	_just_set_record = new_record
	_just_unlocked_difficulty = new_unlock
	if _auto:
		print("[autoclient] итог боя с ИИ от сервера: рекорд=%s, открыт блок=%s, unlocked=%d, best=%d"
			% [new_record, new_unlock, Difficulty.unlocked, Difficulty.best])
...
# Только hotseat двух людей за одним устройством. Бой против ИИ локальным БЫТЬ НЕ МОЖЕТ: он идёт
# на сервере (см. _start_ai_match) — иначе исход, от которого зависят прогресс и лидерборд,
# считал бы сам клиент, а значит мог бы его и нарисовать.
...
func _on_auth_ok(login: String, token: String, _rating: int, loadout: Array, difficulty_unlocked: int,
	...
	if not _menu_shown_once:
		...
		_show_menu()
	elif _pending_intent == INTENT_AI:
		# Повторный вход затребован кнопкой «Бой» (см. _start_ai_match) — начинаем бой с ИИ.
		_start_ai_match()
	else:
...
func _online_plan() -> void:
	...
	# Ход бота определён целиком и сразу (сервер спланирует его, как только придут наши приказы) —
	# пустой слот у него это осознанный «пас», а не «ещё думает». Поэтому показываем ВСЕ слоты
	# соперника занятыми, а не ждём opponent_progress, которого от бота никогда не будет.
	if _ai_match:
...
func _on_online_orders_ready(o: Array) -> void:
	...
	_status("Приказы отправлены. Ход ИИ…" if _ai_match else "Приказы отправлены. Ожидание соперника…")
...
func _on_online_resolution_done() -> void:
	...
	if state.winner >= 0:
		if not _auto:
			...
			get_tree().quit(0)
		return
	# Автопрогон PvP обрывается на 2 раундах (двум ботам играть до победы незачем), а бой с ИИ
	# доигрывается до конца — именно финал проверяет серверный подсчёт исхода и ai_progress_rpc.
	if _auto and not _auto_ai and state.round_num >= 2:
...
func _show_victory() -> void:
	...
	box.add_child(s)
	if _just_set_record:
		_just_set_record = false
		var r := Label.new()
		r.text = "Личный рекорд: %d — вы в лидерборде!" % Difficulty.best
...
# Победа или поражение — с точки зрения того, кто сидит за этим устройством.
#  * серверный матч (PvP и бой с ИИ): играем за board_view.my_player, всё однозначно;
#  * хотсит: за устройством ОБА игрока, проигравшего, которому положен звук поражения, тут нет —
#    кто-то из двоих победил, значит фанфары;
#  * ничья: ни победа, ни поражение — молчим, врать звуком не о чем.
func _play_outcome_sound() -> void:
	if state.winner == Consts.DRAW:
		return
	var mine := (not online) or state.winner == board_view.my_player
