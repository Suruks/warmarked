...
const SCORE_PAD := 7            # отступ под очками игрока (уменьшен — очки ближе к скиллам героя)
const EFFECT_H := 56
const EFFECT_GAP := 4           # отступ от полосы эффектов до нижней панели
...
static var EFFECT_Y: float = 671.0
static var PANEL_TOP: float = 731.0
static var PANEL_W: float = 532.0
static var PANEL_H: float = 461.0
